...
def move(velocity_publisher,speed,distance,is_forward):
    ...
    #declare a Twist message to send velocity command
    velocity_message = Twist()

    #get current location
    global x , y

    x0 = x #save the initial x location
    y0 = y #save the initial y location
    ...
    loop_rate = rospy.Rate(100) #we publish the velocity at 10Hz
...
def rotate(velocity_publisher, angular_speed_degree, relative_angle_degree, clockwise):
    ...
    velocity_publisher.publish(velocity_message) #publishing the message when the  paramerter is updated..
...
def setDesiredOrientation(velocity_publisher, speed_in_degree, desired_angle_degree):
    ...
    print ("relative_angle_degres:", math.degrees(relative_angle_radians))
...
def gridClean(velocity_publisher):
    ...
    setDesiredOrientation(velocity_publisher, 10, math.radians(desired_pose.theta))

    for i in range(5):
        move(velocity_publisher, 2.0, 1.0, True)
        rotate(velocity_publisher, 10, 90, False)
...
if __name__=="__main__":
    try:
        ...
        #rotate(velocity_publisher,10, 110, True)
        #go_to_goal(velocity_publisher,5.0, 5.0)
        #setDesiredOrientation(velocity_publisher, 10, 60)
